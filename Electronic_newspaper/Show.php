    ...
    <body>
        ...
        <section>
            ...
            <?php 
                ...
                if ($type == "cheif_editor"){
                    echo '<form  action="insertCommentAsCheif.php" method="POST" style="margin-top: 35px; margin-left: 38%;">';
                    echo '<input type="hidden" name="aID" value="'.$aID.'">';
                    echo '<input type="submit" name="submit" id="commentbutton" value="comment">';
                    echo '<label>';
                    echo '<input type="text" name="content" style="border: none;background-color: #F0F0F4;  width:27%; padding: 4px; " required placeholder="type a comment"  >';
                    echo '</label>';
                    echo '</form>';
                }
            ?>
            ...
            <section style="margin-left: 25%; margin-right: 25%; border-style: solid; padding: 20px; margin-top: 2%; max-height: 50%;overflow: scroll">
                ...
                <?php
                    ...
                    while($row2 = mysqli_fetch_array($result2)){
                        ...
                        $result3 = mysqli_query($con, "SELECT * FROM comment_creator WHERE Creator_ID = ".$creatorID." AND Table_Creator = 'cheif_editor'");
                        if($row3 = mysqli_fetch_array($result3)){
                            $IDC = $row3['ID_of_creator'];
                        
                        
                        $result4 = mysqli_query($con, "SELECT * FROM cheif_editor WHERE Cheif_ID = ".$IDC."");
                        if($row4 = mysqli_fetch_array($result4))
                        {$name = $row4['Cheif_name'];
        ...
        <section>
            <?php 
                $type = isset($_SESSION['type'])? $_SESSION['type'] : "" ;
                if ($type == "cheif_editor"){
                    echo '<form  action="Show_1.php" method="POST" style="margin-top: 35px; margin-left: 38%;">';
                    echo '<input type="hidden" name="aIDr" value="'.$aID.'">';
                    echo '<input type="submit" name="submit" id="commentbutton" value="remove">';
                    echo '</form>';
                }
